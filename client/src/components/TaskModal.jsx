...
const TaskModal = ({ onClose, onSave, taskToEdit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    await onSave({ ...form, attachments, pendingFiles });
    setSaving(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/75 backdrop-blur-sm animate-fadeIn p-4">
      ...
      <div
        className="w-full max-w-md glass-card rounded-2xl! shadow-2xl flex flex-col"
          ...
          <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <label
                  className={`flex items-center justify-center gap-2 p-3 rounded-xl border-2 border-dashed border-white/10 hover:border-indigo-500/50 hover:bg-indigo-500/5 transition-all cursor-pointer ${uploading ? "opacity-50 pointer-events-none" : ""}`}
                >
                  <input
                    type="file"
                    className="hidden"
